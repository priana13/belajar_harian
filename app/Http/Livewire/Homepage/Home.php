    ...
    public function render()
    {
        ...
        if (Auth::check()) {
            ...
            if($angkatan_aktif){
                ...
                $materi = Belajar::where('user_id', auth()->user()->id)->where('angkatan_id', $angkatan_aktif->angkatan_id)->where('tanggal', date('Y-m-d'))->latest()->first(); 
                
                if($materi){
                    
                    $ujian_harian = JadwalUjian::where('type', 'Harian')->where('angkatan_id', $angkatan_aktif->angkatan_id)->where('urutan', $materi->materi_detail->pertemuan)->first();
