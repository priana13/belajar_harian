...
function initAudioPlayer() {
    ...
    window.track_index = window.track_index || 0;
    window.isPlaying = window.isPlaying || false;
    window.updateTimer = window.updateTimer || null;
    window.curr_track = window.curr_track || document.createElement('audio');
    ...
    window.open_modal = function(id){
        window.showModal();
        window.track_index = id;
        loadTrack(window.track_index);
        playpauseTrack();
    }

    function loadTrack(track_index) {
        let materi = window.track_list[track_index];
        clearInterval(window.updateTimer);
        resetValues();
        window.curr_track.src = materi.path;
        window.curr_track.load();
        track_name.textContent = materi.name;
        track_artist.textContent = materi.artist;
        window.updateTimer = setInterval(seekUpdate, 1000);
        window.curr_track.addEventListener("ended", nextTrack);
    ...
    window.playpauseTrack = function() {
        if (!window.isPlaying) playTrack();
        else pauseTrack();
    }

    function playTrack() {
        window.curr_track.play();
        ...
        if(document.getElementById('playPauseIcon'+window.track_index)) {
            document.getElementById('playPauseIcon'+window.track_index).classList.add('fa-pause');
            document.getElementById('playPauseIcon'+window.track_index).classList.remove('fa-play');
        }
    }

    function pauseTrack() {
        window.curr_track.pause();
        ...
        if(document.getElementById('playPauseIcon'+window.track_index)) {
            document.getElementById('playPauseIcon'+window.track_index).classList.remove('fa-pause');
            document.getElementById('playPauseIcon'+window.track_index).classList.add('fa-play');
        }
    }

    window.nextTrack = function() {
        if (window.track_index < window.track_list.length - 1)
            window.track_index += 1;
        else window.track_index = 0;
        loadTrack(window.track_index);
        playTrack();
    }

    window.prevTrack = function() {
        if (window.track_index > 0)
            window.track_index -= 1;
        else window.track_index = window.track_list.length - 1;
        loadTrack(window.track_index);
        playTrack();
    }

    window.seekTo = function() {
        let seekto = window.curr_track.duration * (seek_slider.value / 100);
        window.curr_track.currentTime = seekto;
    }

    window.setVolume = function() {
        window.curr_track.volume = volume_slider.value / 100;
    }

    function seekUpdate() {
        let seekPosition = 0;
        if (!isNaN(window.curr_track.duration)) {
            seekPosition = window.curr_track.currentTime * (100 / window.curr_track.duration);
            seek_slider.value = seekPosition;
            let currentMinutes = Math.floor(window.curr_track.currentTime / 60);
            let currentSeconds = Math.floor(window.curr_track.currentTime - currentMinutes * 60);
            let durationMinutes = Math.floor(window.curr_track.duration / 60);
            let durationSeconds = Math.floor(window.curr_track.duration - durationMinutes * 60);
