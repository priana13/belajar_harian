...
<div>
    ...
    <h2 class="text-2xl font-bold text-center text-gray-700 mt-6">Formulir Pendaftaran</h2>       
        ...
        <form action="{{ route('register.store') }}" method="post" class="px-3">
            ...
            {{-- <div class="grid grid-cols-[30%_67%] gap-3 hidden">
                <div>
            ...
            <p class="my-2">Umur</p>
            <input name="umur" class="bg-gray-100 w-full border border-gray-400 rounded-xl " type="number" placeholder="Berapa tahun umur Anda saat ini" value="{{ old('umur') }}">
            @error('umur')
            <span class="text-red-400 text-sm">{{ $message }}</span>  
            @enderror

            <p class="my-2">Provinsi</p>
            <select wire:model="provinsi_id" name="provinsi" class="bg-gray-100 w-full border border-gray-400 rounded-xl " type="text" placeholder="provinsi" value="{{ old('provinsi') }}">
